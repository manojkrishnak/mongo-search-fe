...
function App() {
  ...
  const fetchStaysInThatCity = async () => {
    const res = await fetch(`https://mongo-search-be.adaptable.app/search-selected-city?city=${usersCity}`, {
      headers: {
  ...
  const fetchOperatingAt = async () => {
    const res = await fetch(`https://mongo-search-be.adaptable.app/operating-at`, {
      headers: {
        'Content-Type': 'application/json'
      }
    });
